...
parser.add_argument('--prob', default=0.3, type=float,
                    help='Probability assigned to non-true classes.')

parser.add_argument('--tau', default=10, type=float,
                    help='temperature parameter.')

parser.add_argument('--alpha', default=1, type=float,
                    help='alpha is proportion of soft-loss.')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for ii in range(num_classes):
        sim_vec = sim_mat[ii,:]
        exps = np.exp(mult_dotp * sim_vec)
        # ignore similarity of correct class with itself
        exps[ii] = 0
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion, optimizer, epoch, args, num_classes, q_dist)
...
def train(train_loader, model, criterion, optimizer, epoch, args, num_classes, q_dist):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        for ii in range(target.shape[0]):
                y_train_sl[ii,:] = q_dist[target[ii],:]
